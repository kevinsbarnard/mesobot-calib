...
double calibStereo(
	vector<String>& urls1, // List of paths for first camera
	vector<String>& urls2, // List of paths for second camera
	...
	vector<vector<Mat>>& undistortRectifyMaps, // Undistort and rectify maps
	const bool doCalibration, // Do calibration (true) or read from file (false)
	...
	if (doCalibration) {
		...
		imageSize = validatePairs(urls1, urls2, imagePoints, validPairs, chessboardSize, chessboardFlags);
		...
		while (overallError > epsilon) {
			...
			for (int i = 0; i < validPairs.size(); i++) {
				if (validPairs[i]) {
					validUrls1.push_back(urls1[i]);
					validUrls2.push_back(urls2[i]);
		...
		cout << "[INFO] Reading calibration info from 'calib_settings.yml'" << endl;

		fs.open("calib_settings.yml", FileStorage::READ);
		if (fs.isOpened()) {
			fs["overallError"] >> overallError;
			fs["errors"] >> errors;
			fs["imageSize"] >> imageSize;
			fs["M1"] >> cameraMatrices[0][0];
			fs["D1"] >> cameraMatrices[0][1];
			fs["M2"] >> cameraMatrices[1][0];
			fs["D2"] >> cameraMatrices[1][1];
			fs["R"] >> RTEF[0];
			fs["T"] >> RTEF[1];
			fs["E"] >> RTEF[2];
			fs["F"] >> RTEF[3];
